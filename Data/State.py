...
class State:
    def __init__(self, **kwargs):
        ...
        * 3. move by note: father, note
        ...
        if 'view' in kwargs:
            ...
            self.turn = Helper.get_opposite(self.father.turn)
            ...
            if 'note' in kwargs:
                ...
                positions = Helper.make_move_by_note_and_get_positions(self.note, self.pieces,
                                                                       self.view, self.father.turn)
            else:
                self.note = Helper.make_move_and_get_notation(kwargs['clicked_piece'].position, kwargs['destination'],
    ...
    def starting(self, view: str):
        ...
        self.view = view
        self.turn = 'white'
        self.pieces, self.kings = Helper.set_board(view)
        self.privileged_pawns = []
        self.moves_played = 0
        self.PGN = 'PNG: '
        self.__memo_unit = {
    ...
    def update_memo(self, key, value):
        mongo.offline_book.remove(self.__memo_unit)
        self.__memo_unit[key] = value
        mongo.offline_book.append(self.__memo_unit)
        mongo.active_book.update_one({'_id': self.__memo_unit['_id']}, {'$set': {key: value}})

    def get_text(self, name):
        if not hasattr(self, 'kings'): return ''
        if name in self.__memo_unit.keys(): return self.__memo_unit[name]
        if name == 'PNG': return self.PGN
        raise Exception()
    ...
    def get_new_state_if_possible(self, clicked_piece, destination, promote_to):
        if Helper.is_move_in(clicked_piece.position, destination, self.pieces, self.view, self.kings, promote_to,
